...
GstElement* pipeline;
struct conn_io * recv_conn_io = NULL;
...
static gboolean recv_cb (GIOChannel *channel, GIOCondition condition, gpointer data) {
    ...
    while (1) {
        ...
        if (quiche_conn_is_established(conn_io->conn) && !ready_to_send) {
    ...
    HASH_ITER(hh, conns->h, conn_io, tmp) {
        flush_egress(conn_io);//send ack frame, etc
...
void goHandlePipelineBuffer(void *buffer, int bufferLen, int pipelineId) {
    ...
    if (pipelineId == 0) {
        ...
        if (quiche_conn_is_established(recv_conn_io->conn)) {
            if (use_dgram) {
                quiche_conn_dgram_send(recv_conn_io->conn, (uint8_t *) buffer, bufferLen);
                printf("dgram_send %d bytes\n", bufferLen);
                flush_egress(recv_conn_io);
            }
            else {
                //increase stream id
                static int cur_stream_id = 5;
                int size = quiche_conn_stream_send(recv_conn_io->conn, cur_stream_id, (uint8_t *) buffer, bufferLen, TRUE);
                printf("stream_send %d bytes on stream id %d\n", size, cur_stream_id);
                cur_stream_id += 4;
                flush_egress(recv_conn_io);
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    gstreamer_send_destroy_pipeline(pipeline);
    freeaddrinfo(local);
