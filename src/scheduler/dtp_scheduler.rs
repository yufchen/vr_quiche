...
impl Scheduler for DtpScheduler {
        ...
        if len <= 0 {
            for i in 0..blocks_vec.len() {
                ...
                if block.remaining_size > 0 {
                    ...
                    let remaining_time: f64 = tempddl as f64 - passed_time as f64 - one_way_delay - (tempsize as f64 / (pacing_rate * 1024.0)) * 1000.0; // Bytes / (KB/s) * 1000. (ms)
                    ...
                    if remaining_time >= 0.0 {
                        ...
                        let weight: f64 = (1.0 * remaining_time / ddl as f64) / (1.0 - tempprio as f64 / self.max_prio as f64);
    ...
    fn should_drop_block(
        ...
        let passed_time = current_time - block.block_create_time;
        debug!("dtp should_drop_block: passed time: {}, block dll: {}", passed_time, block.block_deadline);
